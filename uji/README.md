...
| `uji-jscroot.mjs` | Adopsi jscroot + halaman depan: tiap halaman memuat rantai modulnya, header `login` terkirim di halaman terlindungi dan TIDAK di verifikasi publik, 401 membuang sesi, data tidak jadi HTML. halaman depan tidak mengalihkan dan tidak menyentuh rute ber-token, berita digambar sebagai teks (judul ber-HTML tidak jadi elemen) dan API berita yang mati tidak merusak sisa halaman, dan **rantai QR `/verifikasi/<token>` → `404.html` → `?token=` masih utuh** (dijalankan di atas server tiruan GitHub Pages di port 5174, karena server statis biasa tidak menyajikan `404.html`). **Tidak butuh backend hidup** — jawabannya dipalsukan lewat `page.route` |
